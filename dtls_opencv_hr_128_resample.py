...
class DTLS(nn.Module):
    ...
    @torch.no_grad()
    def sample(self, batch_size=16, img=None, t=None, imgname=None, mask=None, idx=None, result_folder=None, resample=100):
        if t == None:
        ...
        img = self.inpaint_func(img.clone(), mask)
        utils.save_image((img+1)/2, str(result_folder / f'temp.png'))
        blur_img = self.transform_func_sample(img.clone(), self.size_list[t])
        utils.save_image((blur_img+1)/2, str(result_folder / f'temp.png'))
        os.remove(str(result_folder / f'temp.png'))
