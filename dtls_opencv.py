...
class DTLS(nn.Module):
    ...
    def sample(self, batch_size=16, img=None, t=None, imgname=None, mask=None, idx=None, result_folder=None):
        ...
        img_compare = torch.cat((img_compare, ((F.interpolate(img, size=128, mode='bilinear', antialias=True)+1)/2).to("cpu")), dim=0)
        img_t = self.inpaint_func(img.clone(), mask)
        img_compare = torch.cat((img_compare, ((F.interpolate(img_t, size=128, mode='bilinear', antialias=True)+1)/2).to("cpu")), dim=0)
        
        plt.imshow((((img_t+1)/2).permute(0, 2, 3, 1).detach().cpu().numpy() * 255).astype(np.uint8).squeeze(0))
        plt.show()
