...
class DTLS(nn.Module):
    ...
    def sample(self, 
        ...
        img_compare = torch.cat((img_compare, (((F.interpolate(img, size=128, mode='bilinear', antialias=True)+1)*127.5)/255.0).to("cpu")), dim=0)
        
        inverted_mask = 1-oriMask
        
        model_kwargs["gt"] = img
        model_kwargs["gt_keep_mask"] = mask
        inpaintedImg = sample_fn(
                    model_fn,
        ...
        img = ((inpaintedImg+1)*127.5)/255.0
        img = img * 2 - 1
        
        img_compare = torch.cat((img_compare, ((F.interpolate(img_t, size=128, mode='bilinear', antialias=True)+1)/2).to("cpu")), dim=0)
        
        blur_img = self.transform_func_sample(img_t, self.size_list[t])
        ...
        while (t):
            ...
            R_x = (oriImg.clone() * oriMask) + (R_x * inverted_mask)
...
class Trainer(object):
    ...
    def evaluation(self):
        ...
        conf.update(yamlread('confs/test_128_inpaint.yml'))
        ...
        def model_fn(x, t, y=None, gt=None, **kwargs):
            assert y is not None
            return diff_model(x, t, y if conf.class_cond else None, gt=gt)
        ...
        hq_img_set = torch.tensor([])
        for idx, (batch, img_path, mask_path) in enumerate(zip(iter(dl), list_img, list_mask)):
            ...
            if conf.cond_y is not None:
                classes = torch.ones(batch_size, dtype=torch.long, device=self.device)
            ...
            # read image
            oriImg = cv2.imread(img_path, cv2.IMREAD_COLOR).astype(np.float32) / 255.
            oriImg = torch.from_numpy(np.transpose(oriImg[:, :, [2, 1, 0]], (2, 0, 1))).float()
            oriImg = torch.clamp((oriImg * 255.0).round(), 0, 255) / 255.
            oriImg = oriImg.unsqueeze(0).to(self.device)
            oriImg = oriImg * 2 - 1

            oriMask = cv2.imread(mask_path, cv2.IMREAD_GRAYSCALE).astype(np.float32) / 255.
            oriMask = torch.from_numpy(oriMask).float()
            oriMask = oriMask.unsqueeze(0).unsqueeze(0).to(self.device)
            
            img_sr = self.ema_model.sample(
                batch_size=1,
                img=model_kwargs["gt"],
