...
class DTLS(nn.Module):
    ...
    @torch.no_grad()
    def sample(self, batch_size=16, img=None, t=None, imgname=None, mask=None, idx=None, result_folder=None):
        if t == None:
            t = self.num_timesteps
        img_compare = torch.tensor([])
        dsImg = F.interpolate(img.clone(), size=16, mode="bicubic", antialias=True)
        
        dsMask = F.interpolate(mask.clone(), size=16, mode="bicubic", antialias=True)
        dsMask = torch.where(dsMask < 0.95, torch.tensor(0.0), torch.tensor(1.0))
        inverted_mask = 1-mask
        
        img_compare = torch.cat((img_compare, (((F.interpolate(dsImg, size=128, mode="bicubic", antialias=True))+1)/2).to("cpu")), dim=0)
        img_t = self.inpaint_func(dsImg.clone(), dsMask)
        img_compare = torch.cat((img_compare, (((F.interpolate(img_t, size=128, mode="bicubic", antialias=True))+1)/2).to("cpu")), dim=0)
