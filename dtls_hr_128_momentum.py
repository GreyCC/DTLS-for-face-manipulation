...
class DTLS(nn.Module):
    ...
    @torch.no_grad()
    def sample(self, batch_size=16, img=None, t=None, imgname=None, mask=None, idx=None, result_folder=None, resample=0):
        if t == None:
        ...
        if resample == 0:
            while (t):
                ...
                if previous_x_s0 is None:
                    momentum_l = 0
                    gt_momentum_l = 0
                    predict = self.denoise_fn(img_t, step)
                    previous_x_s0 = predict
                    previous_gt = img.clone()
                    utils.save_image(((predict+1)/2), str(result_folder /  f'after_predict_{idx}.png'))
                    utils.save_image((((predict * inverted_mask)+1)/2), str(result_folder /  f'predict_part_{idx}.png'))
                    ...
                    gt_momentum_l = self.transform_func_sample(gt_momentum, current_step)
                    utils.save_image((((momentum_l)+1)/2), str(result_folder /  f'momentum_pred_{idx}.png'))
                    utils.save_image((((gt_momentum_l)+1)/2), str(result_folder /  f'momentum_gt_{idx}.png'))
                    utils.save_image(((((momentum_l+gt_momentum_l))+1)/2), str(result_folder /  f'combine_momentum_{idx}.png'))
                    utils.save_image((((img_t+(momentum_l+gt_momentum_l))+1)/2), str(result_folder /  f'momentum_combine_{idx}.png'))
                    predict = self.denoise_fn(img_t+(momentum_l+gt_momentum_l), step)
                    utils.save_image(((predict+1)/2), str(result_folder /  f'after_predict_{idx}.png'))
                    utils.save_image((((predict * inverted_mask)+1)/2), str(result_folder /  f'predict_part_{idx}.png'))
                    utils.save_image((((img.clone() * mask)+1)/2), str(result_folder /  f'gt_part_{idx}.png'))

                #Combine predict and gt img
                R_x = (img.clone() * mask) + (predict * inverted_mask)
                momentum = (previous_x_s0 * inverted_mask) - (predict * inverted_mask)
                gt_momentum = (previous_gt * mask) - (predict * mask)
                
                utils.save_image((((previous_x_s0 * inverted_mask)+1)/2), str(result_folder /  f'momentumPrevPred_{idx}.png'))
                utils.save_image((((predict * inverted_mask)+1)/2), str(result_folder /  f'momentumPred_{idx}.png'))
                utils.save_image((((momentum)+1)/2), str(result_folder /  f'momentum_pred_before_{idx}.png'))
                
                utils.save_image((((previous_gt * mask)+1)/2), str(result_folder /  f'momentumPrevGT_{idx}.png'))
                utils.save_image((((predict * mask)+1)/2), str(result_folder /  f'momentumGT_{idx}.png'))
                utils.save_image((((gt_momentum)+1)/2), str(result_folder /  f'momentum_gt_before_{idx}.png'))
                
                utils.save_image(((R_x+1)/2), str(result_folder /  f'combine_{idx}.png'))
                #plt.imshow(((img_t+(momentum_l+gt_momentum_l)).permute(0, 2, 3, 1).detach().cpu().numpy() * 255).astype(np.uint8).squeeze(0))
                #plt.show()
                previous_x_s0 = R_x
